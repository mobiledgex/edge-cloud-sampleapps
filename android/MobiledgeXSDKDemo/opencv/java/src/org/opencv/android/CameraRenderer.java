...
public class CameraRenderer extends CameraGLRendererBase {
    ...
    protected synchronized void openCamera(int id) {
        ...
        if (id == CameraBridgeViewBase.CAMERA_ID_ANY) {
            Log.d(LOGTAG, "Trying to open com.mobiledgex.sdkdemo.camera with old open()");
            try {
                mCamera = Camera.open();
            }
            catch (Exception e){
            ...
            if(mCamera == null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                ...
                for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                    Log.d(LOGTAG, "Trying to open com.mobiledgex.sdkdemo.camera with new open(" + camIdx + ")");
                    try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                ...
                if (mCameraIndex == CameraBridgeViewBase.CAMERA_ID_BACK) {
                    Log.i(LOGTAG, "Trying to open BACK com.mobiledgex.sdkdemo.camera");
                    ...
                    Log.i(LOGTAG, "Trying to open FRONT com.mobiledgex.sdkdemo.camera");
                    Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
                    for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                        Camera.getCameraInfo( camIdx, cameraInfo );
                ...
                if (localCameraIndex == CameraBridgeViewBase.CAMERA_ID_BACK) {
                    Log.e(LOGTAG, "Back com.mobiledgex.sdkdemo.camera not found!");
                } else if (localCameraIndex == CameraBridgeViewBase.CAMERA_ID_FRONT) {
                    Log.e(LOGTAG, "Front com.mobiledgex.sdkdemo.camera not found!");
                } else {
                    Log.d(LOGTAG, "Trying to open com.mobiledgex.sdkdemo.camera with new open(" + localCameraIndex + ")");
                    try {
        ...
        if(mCamera == null) {
            Log.e(LOGTAG, "Error: can't open com.mobiledgex.sdkdemo.camera");
            return;
        }
    ...
    public synchronized void setCameraPreviewSize(int width, int height) {
        ...
        if (psize.size() > 0) {
            ...
            for (Size size : psize) {
                ...
                Log.d(LOGTAG, "checking com.mobiledgex.sdkdemo.camera preview size: "+w+"x"+h);
                if ( w <= width && h <= height &&

...
var SIMULATELAGTIMEINMS = 250; // In milliseconds.
...
wsServer.on('request', function(request) {
  ...
  // probably should store this globally on the server somewhere to reconnect clients
  // (and expire them via timestamped timer interval or on('close').
  ...
  playerkey = request['httpRequest']['headers']['sec-websocket-key'];
  ...
  // Update connections:
  connectedClients[playerkey] = connection;
  ...
  connection.on('message', function(message) {
    ...
    if (message.type === 'utf8') {
	  ...
	  gameMessage = JSON.parse(message.utf8Data);
      console.log("gameMessage: " + message.utf8Data);
...
// Attach broadcaster:to all clients:
var count = 0;
...
function updateGameStates(alias, playerGameState) {
	// Lookup gameId to get gameState records.
	// append and update data between the 2 clients.
	// send new gamestate to both clients.

...
open class UISideMenuNavigationController: UINavigationController {
    ...
    override open func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
      ...
      ///  let zzz = UIResponder.willChangeStatusBarFrameNotification
        NotificationCenter.default.removeObserver(self.transition, name: UIApplication.willChangeStatusBarOrientationNotification, object: nil)   // JT 18.09.22
        coordinator.animate(alongsideTransition: { (context) in
            self.transition.presentMenuStart()
        }) { (context) in
            NotificationCenter.default.addObserver(self.transition, selector:#selector(SideMenuTransition.handleNotification), name: UIApplication.willChangeStatusBarFrameNotification, object: nil)   // JT 18.09.22
        }
    }
